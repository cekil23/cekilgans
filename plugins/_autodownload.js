...
handler.all = async function (m, { isPrems }) {
    ...
    if (/^.*tiktok/i.test(m.text)) {
        ...
        await this.sendFile(m.chat, json.nowm, '', '©  SHINN - BOT', m)
    }
    ...
    if (/^.*cocofun/i.test(m.text)) {
        ...
        await m.reply(wait)
        // m.reply(util.format(json))
        await this.sendFile(m.chat, json.download, '', '©  SHINN - BOT', m)
    }
    ...
    if (/^.*(pinterest.com\/pin|pin.it)/i.test(m.text)) {
        pin(url).then(async res => {
            ...
            await m.reply(wait)
            m.reply(util.format(json))
            await this.sendFile(m.chat, json.data.url, '', '©  SHINN - BOT', m)
        }).catch(_ => _)
    }
    ...
    if (/^.*twitter.com\//i.test(m.text)) {
        twitter(url).then(async res => {
            ...
            for (let { url } of json.data) {
                this.sendFile(m.chat, url, 'ig' + (/mp4/i.test(url) ? '.mp4' : '.jpg'), '©  SHINN - BOT', m)
            }
        }).catch(_ => _)
    }
